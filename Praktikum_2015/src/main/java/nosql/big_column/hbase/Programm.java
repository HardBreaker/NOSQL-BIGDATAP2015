...
//		hbaseClient.fillDatabase();
	...
	private List<String> gibCityAndLand(String plz) {
		try {
			...
			City city = hbaseClient.getCityByPLZ(plznb);
			if (city == null) {
	...
	private int gibPLZZuCity(String cityName) {
		...
		List<String> list = hbaseClient.getListFromCityname(upperCaseCityName);
		System.out.println("Gefunden PLZ für " + cityName + ": "
				+ list.toString());
		return 0;
	}
